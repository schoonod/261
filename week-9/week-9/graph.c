...
int DFS(Graph* g, Vertex* source, Vertex* destination){
    if(source = destination) return1;
    struct cirListDeque *cStack = malloc(sizeof(struct cirListDeque));
    initCirListDeque(cStack);
    clearVisited(g);
    
    addBackCirListDeque(cStack, source);
    
    // While there are still places to visit
    while (!isEmptyCirListDeque(cStack)){
        Vertex *currentVertex = backCirListDeque(cStack);
        // pop the back
        removeBackCirListDeque(cStack);
        ...
        if (currentVertex == destination){
            ...
            for (int i = 0; i < currentVertex->numNeighbors; i++){
                if (currentVertex->neighbors[i]->isVisited == 0){
                    // if we have nto visited the neighbors, add them to the back of the deque
                    addBackCirListDeque(cStack, currentVertex->neighbors[i]);
                    // neighbor has been 'visited'
                    currentVertex->neighbors[i]->isVisited = 1;
                }
            }
    ...
    int BFS(Graph* g, Vertex* source, Vertex* destination){
        struct cirListDeque *cQueue = malloc(sizeof(struct cirListDeque)); //stack container for vertices
        initCirListDeque(cQueue);
        clearVisited(g); 
        
        addBackCirListDeque(cQueue, source);
        
        while (!isEmptyCirListDeque(cQueue)){
            Vertex *currentVertex = frontCirListDeque(cQueue);
            // pop the front
            removeFrontCirListDeque(cQueue);
            ...
            for (int i = 0; i < currentVertex->numNeighbors; i++){
                if (currentVertex->neighbors[i]->isVisited == 0){
                    addBackCirListDeque(cQueue, currentVertex->neighbors[i]);
                    currentVertex->neighbors[i]->isVisited = 1;
                }
            }
            currentVertex->isVisited = 1;
        }
        return 0;//no path to destination found
    }
